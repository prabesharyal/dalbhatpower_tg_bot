    ...
    for url in entries:
        ...
        if check == True:
            ...
            with Loader("Uploading Tiktok/YtShort : ","Tiktok/YtShort Upload Success"):
                ...
                await update.message.reply_video(video=open(filename, 'rb'),duration=video_duration, caption=CAPTION, disable_notification=True, width=video_dimensions.get('width', 0), height=video_dimensions.get('height', 0),thumbnail=open(video_thumbnail_path,'rb'), parse_mode='HTML', supports_streaming=True)
            await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="cancel")
            os.remove(filename)
    print("%50s"%"Done\n")
        ...
        if check == True:
            if is_file_size_less_than_50mb(filename):
                with Loader("Uploading Short Audio : ","Short Audio Upload Success"):
                    ...
                    await update.message.reply_audio(audio=open(filename, 'rb'),duration=audio_duration, caption=CAPTION,disable_notification=True,parse_mode='HTML')
                    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action="cancel")
            else :
                try:
