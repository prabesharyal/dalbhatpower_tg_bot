...
async def tweet_dl(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        only_necesssary_regex = r'((twitter)\.(com)\/([\w]+)\/(status)\/[\d]+)'
                            ...
                            if is_file_size_less_than_50mb(filename):
                                ...
                                with Loader("Uploading Tweet Short Video : ","Tweeet Short Video Upload Success"):
                                    try:
                                        ...
                                        await update.message.reply_video(video=open(filename, 'rb'),allow_sending_without_reply=True,duration=video_duration, caption=CAPTION, disable_notification=True, width=video_dimensions['width'], height=video_dimensions['height'],thumb=open(video_thumbnail_path,'rb'), parse_mode='HTML', supports_streaming=True)
